...
public class EraseObject extends World<Tile>
{
    ...
    private void erase( Grid<Tile> gr, int row, int col )
    {
        ...
        {
            ArrayList<Location> list = gr
                .getOccupiedAdjacentLocations( new Location( row, col ) );
            gr.get( new Location( row, col ) ).setColor( ERASE_COLOR );
            for ( int i = 0; i < list.size(); i++ )
            {
                if ( gr.get( list.get( i ) ).getColor() == ERASE_COLOR )
                {
                    list.remove( i );
                    i--;
                }
            ...
            for ( int i = 0; i < list.size(); i++ )
            {
                gr.get( list.get( i ) ).setColor( ERASE_COLOR );
                erase( gr, list.get( i ).getRow(), list.get( i ).getCol() );
            }
